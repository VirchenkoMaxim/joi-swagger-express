 ...
 */
module.exports = function buildRouteDefinition(
  method,
  routePath,
  routeOpts,
  swaggerDef,
  ...
  if (routeOpts.responses) {
    apiDef.responses = _buildResponses(
      routeOpts.responses,
      method,
      routePath,
  ...
  if (Array.isArray(responses)) {
    for (let i = 0; i < responses.length; i++) {
      if (typeof responses[i] === 'number' || !isNaN(responses[i])) {
        ...
        throw new EJSError(
          `Invalid response property: ${responses[i]}`,
          method,
          routePath,
        );
      }
    ...
    for (const k in responses) {
      ...
      if (!responses) {
        ...
        if (responseStructures && responseStructures[httpCode]) {
          responsesDefinition[httpCode] = cloneDeep(
            responseStructures[httpCode],
          );
          responsesDefinition[httpCode].schema.properties.data =
            responses[httpCode];
        } else {
